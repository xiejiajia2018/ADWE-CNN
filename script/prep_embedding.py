...
def gen_np_embedding(fn, word_idx_fn, out_fn, dim=300):
    ...
    # embedding = np.random.uniform(-0.025, 0.025, (len(word_idx)+2, dim))
    with open(fn) as f:
        for l in f:
            rec=l.rstrip().split(' ')
...
parser.add_argument('--emb_dir', type=str, default="/home/summer/xiejiajia/")
parser.add_argument('--out_dir', type=str, default="../data/prep_data/")
parser.add_argument('--gen_emb', type=str, default="gen.vec")
parser.add_argument('--laptop_emb', type=str, default="laptop_emb_300.vec")
...
parser.add_argument('--domain_dim', type=int, default=300)
